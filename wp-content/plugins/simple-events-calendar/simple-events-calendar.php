...
elseif(isset($_POST['update_event'])) {
	...
	$editEvent['event_label'] = strtolower(str_replace(" ", "", $event['event_label']));
...
<div class="wrap">
	<h2 id="plugin-title">Simple Events Calendar <span> <?php _e(' by <a class="pluginhelp" href="http://www.studiostacks.com/plugins/simple-events-calendar" rel="help">Jerry G. Rietveld</a>',SE_TEXTDOMAIN);?></span></h2>
    <div class="donate">
    
<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
<input type="hidden" name="cmd" value="_s-xclick">
<input type="hidden" name="hosted_button_id" value="CVSFHBLEA7BEC">
...
<img alt="" border="0" src="https://www.paypalobjects.com/WEBSCR-640-20110306-1/en_US/i/scr/pixel.gif" width="1" height="1">
    ...
    <h4 class="SE_settings">+ <?php _e('Advanced settings',SE_TEXTDOMAIN);?></h4>
    <div id="settings">
...
?>
        <h3><?php _e('Edit event:',SE_TEXTDOMAIN);?></h3>
    <table>
    <tbody>
    <form method="post">
<?php
...
<?php } else { // if not editing an existing event then add a new one ?>
        <h3><?php _e('Add new event:',SE_TEXTDOMAIN);?></h3>
    <table>
    <tbody>
    <form method="post">
<?php
...
if(count($myevents) > 0) { ?>
    ...
    <h3><?php _e('Current events:',SE_TEXTDOMAIN);?></h3>
...
foreach($myevents as $details) { ?>
        ...
        <td class="eventlocation">
			<?php if($details['event_loc'] != "" && $details['event_loc_url'] != "") {
				echo '<span class="iconsprite locationicon"></span> <a class="eventlinks" href="'.httpprefix($details['event_loc_url']).'">'.$details['event_loc'].'</a>'; 
			} elseif($details['event_loc'] != "" ) {
				echo '<span class="iconsprite locationicon"></span> '.$details['event_loc'];
			} elseif($details['event_loc_url'] != "" ) {
				echo '<span class="iconsprite locationicon"></span> <a class="eventlinks" href="'.httpprefix($details['event_loc_url']).'">'.shortenstring(httpprefix($details['event_loc_url']),30).'</a>'; 				
			} else {
				echo '<span class="iconsprite nolocationicon"></span>';
			}
			...
			<?php if($details['event_label'] != "") { ?>
        		<span class="iconsprite labelicon"></span><span class="eventlabel" title="<?php _e('When including this label on your page when displaying events, only events with this label will be displayed',SE_TEXTDOMAIN);?>"><?php echo $details['event_label'];?></span>
			<?php } else { ?>
                    <span class="iconsprite nolabelicon"></span>
            <?php } ?>
            </td>
            <td>
			<?php if($details['event_url'] != "") {
				echo '<span class="iconsprite linkicon"></span> <a class="eventlinks" href="'.httpprefix($details['event_url']).'" title="'.httpprefix($details['event_url']).'">'.shortenstring(httpprefix($details['event_url']),30).'</a>';
			} else {
...
function displayevents( $atts ) {
	...
	foreach ($allevents as $event) {
		...
		if(isset($atts['moreinfo']) && $atts['moreinfo'] == "no" ) {
			...
			$evt_url = '<div class="info-link"><a class="url" href="'.httpprefix($event['event_url']).'" target="'.$targetInf.'">'.__('More information...',SE_TEXTDOMAIN).'</a></div>';		
		}
		...
		if(isset($atts['tweet']) && $atts['tweet'] == "no" || get_option('SE_twitter') == "no") {
			...
			$evt_twt = '<div><a href="http://twitter.com/share" class="twitter-share-button" data-url="'.httpprefix($event['event_url']).'" data-text="'.stripslashes($event['event_title']).' '.$shorttime.' in '. $event['event_loc'] .'" data-count="none" data-via="wpsec">Tweet</a></div>';
		}
		...
		stripslashes(nl2br($event['event_desc'])).
	...
	if(count($allevents) > 0) $items = implode($the_events); else $items = "There are no events to display";
	return($items);
}
...
function SE_admin_scripts() {	
	wp_enqueue_script('textcounter', SE_PLUGIN . 'simple-events-calendar.js', '', SE_VERSION);
}	
add_action( 'admin_enqueue_scripts', 'SE_admin_scripts' );