 ...
 */
function ai1ecdm_debug( /** polymorphic arg list **/ ) {
...
function ai1ec_database_migration() {
	include_once AI1EC_PATH . DIRECTORY_SEPARATOR . 'lib' .
		DIRECTORY_SEPARATOR . 'database' .
		DIRECTORY_SEPARATOR . 'datetime-migration.php';
	$migrator = new Ai1ecdm_Datetime_Migration();
	$success  = false;
	if ( ! $migrator->is_change_required() || $migrator->execute() ) {
		$success = true;
	}
	unset( $migrator );
	return $success;
...
add_filter( 'ai1ec_perform_scheme_update', 'ai1ec_database_migration' );
